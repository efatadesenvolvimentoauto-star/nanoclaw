...
function listMemoryFiles() {
  const result: { group: string; label: string; files: { name: string; path: string }[] }[] = [];

  const globalFiles = collectMdFiles(GLOBAL_DIR);
  if (globalFiles.length) result.push({ group: 'global', label: 'Global', files: globalFiles });
    ...
    if (entry.isFile() && entry.name.endsWith('.md')) {
      ...
      for (const f of fs.readdirSync(subDir, { withFileTypes: true })) {
        if (f.isFile() && f.name.endsWith('.md')) {
          files.push({ name: `${entry.name}/${f.name}`, path: path.join(subDir, f.name) });
        }
      }
    }
  }
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/files' && req.method === 'GET') {
    json(res, listMemoryFiles()); return;
  }
